...
import type { PrimoSearchResult } from '../src/primo/types.js';
...
describe('primoSearchTool', () => {
  it('returns catalog metadata for successful searches', async () => {
    const resultPayload: PrimoSearchResult = {
      info: {
        total: 2,
        totalLocal: 1,
        totalConsortium: 4,
    ...
    const response = await primoSearchTool.handler(
      { value: 'example title', field: 'title', operator: 'exact', limit: 2 },
      undefined
    );

    expect(searchPrimoMock).toHaveBeenCalledWith(
      expect.objectContaining({ query: 'title,exact,"example title"', limit: 2 })
    );
    ...
    const [firstItem] = parsed.items as any[];
  ...
  it('returns error payloads when Primo calls fail', async () => {
    ...
    const response = await primoSearchTool.handler({ value: 'broken' }, undefined);
...
describe('primoSearchTool with defaults', () => {
  it('uses default field and operator when not provided', async () => {
    const resultPayload: PrimoSearchResult = {
      info: {
        total: 1,
        firstResult: 1,
    ...
    const response = await primoSearchTool.handler({ value: 'test query' }, undefined);

    expect(searchPrimoMock).toHaveBeenCalledWith(
      expect.objectContaining({ query: 'any,contains,"test query"' })
    );
    expect(response.isError).toBeUndefined();
  });
});
